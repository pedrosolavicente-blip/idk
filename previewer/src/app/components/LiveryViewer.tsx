...
function ModelListItem({ model, selected, onClick }: { model: VehicleModel; selected: boolean; onClick: () => void }) {
  return (
    <button
      onClick={onClick}
      className={`w-full text-left rounded-lg px-3 py-2.5 transition-all duration-300 ease-out transform hover:scale-[1.02] ${
        selected
          ? 'bg-gradient-to-r from-yellow-500/20 to-transparent border border-yellow-500/50 text-white shadow-lg shadow-yellow-500/20 scale-[1.02]'
          : 'border border-white/5 bg-white/5 hover:bg-white/10 hover:border-yellow-500/30 text-zinc-400 hover:text-zinc-200'
      }`}
    >
      <div className="flex items-center gap-2">
        <img 
          src={`/${model.id}-front-left.png`}
          alt={model.name}
          className="w-8 h-8 object-cover rounded border border-yellow-500/30"
          onError={(e) => {
        ...
        <Box 
          size={16} 
          className="text-yellow-500 hidden" 
          strokeWidth={1.5} 
        />
        <p className={`text-xs font-semibold truncate flex-1 ${selected ? 'text-white' : ''}`}>{model.name}</p>
      </div>
    </button>
  );
}
...
function Section({ title, icon: Icon, children, defaultOpen = false }: {
  ...
  return (
    <div className="border-b border-white/5">
      <button
        onClick={() => setOpen(o => !o)}
        className="w-full flex items-center gap-2.5 px-4 py-3.5 text-xs font-bold tracking-wider uppercase text-zinc-300 hover:text-yellow-500 transition-colors group"
      >
        <Icon size={13} style={{ color: '#fbbf24' }} />
        <span className="flex-1 text-left">{title}</span>
        {open
          ? <ChevronDown size={13} className="text-yellow-600 group-hover:text-yellow-500" />
          : <ChevronRight size={13} className="text-yellow-600 group-hover:text-yellow-500" />}
      </button>
...
export default function LiveryViewer({ user, onLogout, onShowDisclaimer }: Props) {
  ...
  return (
    <div className="flex h-screen bg-black text-white overflow-hidden">
      ...
      <div className="relative flex-1 bg-gradient-to-br from-black via-zinc-950 to-black" ref={containerRef}>
          ...
          >
            <div className="bg-[#0f0f0f] border border-white/10 rounded-2xl shadow-2xl w-full max-w-md overflow-hidden">
              ...
              <div className="px-6 pt-6 pb-4 text-center border-b border-white/5">
                ...
                <div className="mt-3 h-px bg-gradient-to-r from-transparent via-[#fbbf24]/40 to-transparent" />
              </div>
              ...
              <div className="px-6 py-5 space-y-3">
                ...
                <div className="rounded-xl border border-white/8 bg-white/3 p-4">
                  ...
                  <p className="text-[10px] text-[#fbbf24] font-semibold uppercase tracking-widest mt-1">Backend Development</p>
                </div>

                {/* Link */}
                <div className="rounded-xl border border-white/8 bg-white/3 p-4">
                  <p className="text-sm font-bold text-white">Link</p>
                  <p className="text-[10px] text-[#fbbf24] font-semibold uppercase tracking-widest mt-1">Frontend Development</p>
                </div>
              </div>
              ...
              <div className="px-6 pb-5">
                <button
                  ...
                  className="w-full text-xs font-bold bg-[#fbbf24] hover:bg-[#ffd700] text-black py-2.5 rounded-xl transition-all shadow-lg shadow-[#fbbf24]/20 hover:shadow-[#fbbf24]/40"
                >
                  Close
                </button>
              </div>
        ...
        <div className="absolute top-4 right-4 z-20">
          <button
            ...
            className="flex items-center gap-2 bg-black/70 hover:bg-black/90 border border-white/10 hover:border-[#fbbf24]/50 text-zinc-300 hover:text-[#fbbf24] text-xs font-bold px-3 py-2 rounded-lg transition-all backdrop-blur-sm"
          >
            ...
            <div className="animate-settings-in mt-2 w-72 bg-[#0a0a0a]/95 border border-white/10 rounded-xl p-4 backdrop-blur-sm shadow-2xl">
              <div className="flex items-center justify-between mb-4">
                ...
                <button
                  ...
                  className="flex items-center gap-1.5 text-[10px] text-zinc-500 hover:text-yellow-500 transition-colors"
                >
                  <RotateCcw size={10} />
                  Reset
                </button>
          ...
          <div className="absolute inset-0 flex flex-col items-center justify-center pointer-events-none gap-3">
            <div className="relative">
              ...
              <div className="absolute inset-0 flex items-center justify-center">
                <div className="w-12 h-12 bg-yellow-500/10 rounded-full blur-xl" />
              </div>
            </div>
          ...
          <div className="absolute bottom-6 right-6 flex flex-col items-end gap-2">
            ...
            <div className="relative">
              <button
                ...
                className="flex items-center gap-2 bg-black/70 hover:bg-black/90 border border-white/10 hover:border-yellow-500/50 text-zinc-300 hover:text-yellow-500 text-[10px] font-bold px-3 py-2 rounded-lg transition-all backdrop-blur-sm w-full justify-between"
              >
                ...
                <div className="absolute bottom-full mb-1.5 right-0 bg-[#111] border border-white/10 rounded-xl shadow-2xl overflow-hidden w-52">
                  {([
                    ...
                    <button
                      ...
                      className="w-full flex items-center px-3 py-1.5 text-[11px] text-zinc-300 hover:text-yellow-500 hover:bg-white/5 transition-all text-left"
                    >
                      {entry.label}
                    </button>
                  )))}
            ...
            <button
              ...
              className="flex items-center justify-center gap-2 bg-yellow-500 hover:bg-yellow-400 text-black text-xs font-bold px-4 py-2.5 rounded-lg transition-all shadow-lg shadow-yellow-500/30 hover:shadow-yellow-500/50 hover:scale-105 w-full"
            >
              <Camera size={14} />
              CAPTURE
            </button>
      ...
      {/* ── Left Livery Panel ── */}
      {selectedModel && (
        <div className="absolute left-4 top-20 w-80 bg-[#0a0a0a]/95 border border-yellow-500/20 rounded-xl backdrop-blur-sm shadow-2xl z-30 animate-slide-in">
          <div className="p-4 border-b border-yellow-500/20">
            <div className="flex items-center justify-between mb-2">
              <p className="text-xs font-bold uppercase tracking-widest text-yellow-500">Livery Textures</p>
              <button
                onClick={() => {}}
            ...
            <p className="text-[10px] text-zinc-400">{selectedModel.name}</p>
          </div>
              ...
              <div key={face} className="space-y-2">
                <div className="flex items-center justify-between">
                  ...
                  <div className="flex items-center gap-1.5 bg-black/40 rounded-lg px-2 py-1 border border-white/10">
                    <button
                      ...
                      className="text-zinc-500 hover:text-yellow-500 text-xs w-4 h-4 flex items-center justify-center font-bold transition-colors"
                    >−</button>
                ...
                {getPanelKeys(face).map(panel => (
                  <div key={panel} className="flex items-center gap-2">
                    ...
                    {textures[panel] ? (
                      <div className="flex-1 flex items-center gap-2 bg-yellow-500/10 border border-yellow-500/30 rounded-lg px-2 py-1.5">
                        <span className="flex-1 text-[10px] text-yellow-500 truncate font-semibold">✓ LOADED</span>
                        <button onClick={() => handleRemoveTexture(panel)} className="text-red-400 hover:text-red-300 text-xs font-bold">✕</button>
                      </div>
                    ) : (
                      <label className="flex-1 cursor-pointer bg-black/40 hover:bg-black/60 border border-white/10 hover:border-yellow-500/50 border-dashed rounded-lg px-2 py-1.5 text-[10px] text-zinc-500 hover:text-zinc-300 flex items-center gap-1.5 transition-all">
                        <Upload size={10} />
      ...
      {/* ── Sidebar ── */}
      <div className="w-64 flex flex-col border-l border-yellow-500/10 bg-[#0a0a0a] overflow-y-auto">

        {/* Header */}
        <div className="px-4 py-6 border-b border-yellow-500/20 bg-gradient-to-b from-yellow-500/5 to-transparent">
          <div className="flex items-center gap-2.5 mb-2">
            <img src={itzzLogo} alt="itzz" className="h-7 w-auto drop-shadow-[0_0_8px_rgba(251,191,36,0.5)]" />
          ...
          <div className="h-0.5 bg-gradient-to-r from-yellow-500 to-transparent mb-2" />
            ...
            {filteredModels.length > 0
              ? (
                  ...
                  {filteredModels.map(m => (
                    <button key={m.id} onClick={() => handleSelectModel(m)}
                      ...
                      <div className="flex-1 flex items-center justify-center" style={{ background:'var(--surface3)', minHeight:56 }}>
                        ...
                        />
                        <Box size={20} strokeWidth={1} style={{ color: selectedModel?.id===m.id ? ACCENT : 'var(--text-4)', opacity:0.5, display:'none' }} />
                      </div>
                ...
                <div className="text-center py-8 text-zinc-600 text-xs">
                  <Box size={28} className="mx-auto mb-2 text-yellow-500 opacity-30" strokeWidth={1.5} />
...
// ...
              ...
              className="flex-1 bg-black/40 border border-white/10 rounded-lg text-xs px-3 py-2 text-zinc-300 placeholder:text-zinc-600 outline-none focus:border-yellow-500/50 min-w-0"
            />
            <button
              onClick={handleSavePreset}
              disabled={!presetName.trim() || savingPreset}
              className="text-[10px] font-bold px-3 py-2 rounded-lg bg-yellow-500 text-black disabled:opacity-40 hover:bg-yellow-400 transition-all shrink-0"
            >
              {savingPreset ? '…' : 'Save'}
            </button>
          </div>
          ...
          {presets.length === 0 ? (
            ...
            <div className="space-y-1.5 mt-1 max-h-52 overflow-y-auto pr-0.5">
              {presets.map(preset => (
                <div
                  key={preset.id}
                  className="group flex items-center gap-1.5 rounded-lg border border-white/5 bg-white/5 hover:border-yellow-500/30 transition-all overflow-hidden"
                >
                  <button
        ...
        <Section title="Vehicle Color" icon={Palette} defaultOpen={true}>
            <div className="flex items-center gap-3">
              <div 
                className="w-12 h-12 rounded-lg border-2 border-white/20 cursor-pointer transition-all shadow-md"
                style={{ background: vehicleColor }}
                onClick={() => {}} // Disabled color picker
              />
              {/* Editable hex field */}
              <div className="flex items-center flex-1 bg-black/40 border border-white/10 rounded-lg text-xs px-3 py-2 gap-1 focus-within:border-yellow-500/50 transition-all">
          ...
          <div className="flex flex-wrap gap-2 mt-3">
            {['#000000','#1a1a2e','#c0392b','#27ae60','#2980b9','#8e44ad','#f39c12','#ecf0f1','#fbbf24','#2c2c2c'].map(c => (
              <button
                key={c}
                onClick={() => handleColorChange(c)}
                style={{ background: c }}
                className="w-6 h-6 rounded-lg border-2 border-white/20 hover:scale-110 hover:border-yellow-500/50 transition-all shadow-md"
                title={c}
              />
            ))}
          </div>
